...
void draw() {
  ...
  if (fft.spectrum[30]>.045 || fft.spectrum[31]>.045 || fft.spectrum[32]>.045 || fft.spectrum[33]>.045 || fft.spectrum[34]>.045) {
    if (fft.spectrum[0]<.16 || fft.spectrum[1]<.16 || fft.spectrum[2]<.16) {
      frameRate(40);
  ...
  } else  if (fft.spectrum[0]>.1 || fft.spectrum[1]>.1 || fft.spectrum[2]>.1) {
    frameRate(300);
    image(bass[frameCount%21], 0, 0, width, height);
  } else {
    frameRate(80);
